...
for ip_address in ip_addresses:
    ...
    # Writes the OLT locations and their temperature to a file
    with open("olt_temperature.txt", "a") as file:
        file.write(f"{olt_locations[count]}: {temperature.split("℃")[0]}\n")
    count = count + 1

with open("olt_temperature.txt", "r") as file:
...
with smtplib.SMTP("smtp.gmail.com", port=587) as connection:
    ...
    connection.sendmail(
        ...
        msg=f"Subject:OLTs Temperature Report - {datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
            f"{contents}\n".encode("utf-8")
    )
